...
export default function HrRegisterForm() {
  ...
  const hrData = { email: email, name: '', phone: '', password: '', companyName: company }
  ...
  useEffect(() => {
    ...
    const hrDataHandler = async () => {
      const resData = await getHrById(urlParam.recruiterId, urlParam.token)
      ...
      {email && company ?
        ...
        >
          <Form.Item label='דוא"ל' name="email" rules={[{ required: true, message: 'בבקשה להקליד מייל' }]}>
            <Input id='test1' onChange={e => setEmail(e.target.value)} disabled={true} defaultValue={email} placeholder="mail@gmail.com" />
          </Form.Item>
          ...
          <Form.Item label="שם ארגון" name="companyName">
            <Input id="companyName" defaultValue={company} disabled={true}/>
          </Form.Item>
