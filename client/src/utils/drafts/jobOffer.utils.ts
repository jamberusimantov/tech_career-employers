...
const err = (serviceName: string, error: string) =>
  `error on ${serviceName}: ${error}`;
...
export const getJobOffer = async (query: object, token: string) => {
  try {
    ...
    return { success: false, error: err("getJobOffer", error) };
  } finally {
  }
};
export const getManyJobOffers = async ( query?: object) => {
  try {
    ...
    return { success: false, error: err("getManyJobOffers", error) };
  } finally {
  }
};
export const getJobOfferById = async (_id: string, token?: string) => {
  ...
  try {
    ...
    return { success: false, error: err("getJobOfferById", error) };
  } finally {
  }
};
export const updateJobOfferById = async (
  query: object,
  _id?: string,
  token?: string
) => {
  if (!_id)
    return { success: false, message: requiredId("updateJobOfferById") };
  try {
    return await updateDocById(collection, body(query), key, _id, token);
  } catch (error) {
    return { success: false, error: err("updateJobOfferById", error) };
  } finally {
  }
};
export const deleteJobOfferById = async (_id: string, token?: string) => {
  if (!_id)
    return { success: false, message: requiredId("deleteJobOfferById") };
  try {
    return await deleteDocById(collection, key, _id, token);
  } catch (error) {
    return { success: false, error: err("deleteJobOfferById", error) };
  } finally {
  }
};
