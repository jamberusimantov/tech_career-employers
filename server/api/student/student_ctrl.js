...
async function updateStudentById(req, res) {
    try {
        ...
        if (tokenChecker(token, res) !== true) return
        if (idChecker(_id, res) !== true) return
        const updateDocSuccessCb = (method = 'strict') => (data) => successHandler(data, res, method)
        const updateDocFailCb = () => failHandler(_id, res)

        const request = async() => {
            const getRes = await updateDoc(studentCollection, student, updateDocSuccessCb(method), updateDocFailCb)
            if (getRes && getRes.error) throw new Error(getRes.error)
        }
